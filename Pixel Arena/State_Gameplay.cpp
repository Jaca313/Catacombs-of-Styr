...
void State_Gameplay::CastRays3DWalls()
{
	...
	for (int r = 0; r < fov * res; ++r) {
		...
		sf::Vertex Line[4];
		double pixelstep = m_winSizeX / fov / res;
		Line[0].position.x = r * pixelstep;
		Line[0].position.y = m_winSizeY - lineOff;
		Line[1].position.x = r * pixelstep;
		Line[1].position.y = m_winSizeY - (lineOff + lineH);
		Line[2].position.x = (r + 1) * pixelstep;
		Line[2].position.y = m_winSizeY - (lineOff + lineH);
		Line[3].position.x = (r + 1) * pixelstep;
		...
		//Iterate to the next vertical drawline
		rayAngle = LogManager::FixAngle(rayAngle - 1.0/res);
	}
...
void State_Gameplay::CastRays3DWalls2()
{
...
void State_Gameplay::CastRaysFloorCeil2()
{
	//Get Window Size
	double WinH = Window->getSize().y;
	double WinW = Window->getSize().x;
	...
	//Pick Textures for Ceiling and Floors
	int TexCeil = 2;
	int TexFloor = 3;
	...
	//FLOOR CASTING
	//Goes bottom -> mid for floor also tiles ceiling by inverting coordinates
	for (int y = 0; y < WinH / 2.0 + 1; y++)
	{
		// rayDir for leftmost ray (x = 0) and rightmost ray (x = w)
		//-sin is due to Y being flipped
		rayDirX0 = cos(LogManager::DegtoRad(pDeg + fov / 2.0));
		...
		//Calculate the difference between two floor cast points
		double dX = floorX2 - floorX;
		double dY = floorY2 - floorY;
		...
		// get the texture coordinate from the fractional part and step the difference texture wraps around
		double tx = texWidth * fX;
		double tx2 = texWidth * (fX + dX);
...
void State_Gameplay::CastRaysFloorCeil3()
{
	//Get Window Size
	double WinH = Window->getSize().y;
	double WinW = Window->getSize().x;
	...
	std::vector<double> FloorRays;
	double angle = 0;
	for (int i = -WinW / 2; i < WinW / 2; i++) {
		angle = atan(i / distancetoProj);
		FloorRays.push_back(LogManager::RadtoDeg(angle));
	}
	...
	for (int y = 0; y < WinH / 2.0; y++)
	{
		...
		for (auto R : FloorRays) {
			rayDirX0 = cos(LogManager::DegtoRad(pDeg + R));
			rayDirY0 = -sin(LogManager::DegtoRad(pDeg + R));
			rayDirX1 = cos(LogManager::DegtoRad(pDeg - R));
			rayDirY1 = -sin(LogManager::DegtoRad(pDeg - R));
	...
	ScreenBuffer->draw(Floor, &Resources->Textures.at(Level.getTexCeil()));
	ScreenBuffer->draw(Ceiling, &Resources->Textures.at(Level.getTexFloor()));
